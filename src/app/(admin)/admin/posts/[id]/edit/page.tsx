...
import { ArrowLeft, Sparkles, Image as ImageIcon, Search, Settings, FileText, Save, Calendar } from "lucide-react"
...
export default function EditPostPage({ params }: { params: { id: string } }) {
  ...
  const [publishedAt, setPublishedAt] = useState<string | Date>("")
    ...
    async function fetchData() {
      try {
        ...
        if (catsRes.ok) {
          ...
          setCategories(catsData.map((c: any) => ({ id: c._id || c.id, name: c.name })))
        }
      } catch (err) {
        setError("Error cargando los datos del artículo")
      } finally {
  ...
  if (initialLoading) {
    return (
      <div className="flex flex-col items-center justify-center h-96 space-y-4">
        ...
        <p className="text-[10px] font-black uppercase tracking-[0.3em] text-muted-foreground animate-pulse">Sincronizando Archivos...</p>
      </div>
    )
  }
    ...
    <div className="max-w-7xl mx-auto space-y-8 pb-20 animate-in fade-in slide-in-from-bottom-8 duration-700">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h2 className="text-3xl font-black italic tracking-tighter uppercase leading-none">Editar Artículo</h2>
            <p className="text-muted-foreground font-bold text-xs uppercase tracking-widest mt-1 italic">Modificando publicación existente</p>
          </div>
        </div>
            ...
            <CardContent className="space-y-8 pt-8">
              ...
              <div className="space-y-3">
                ...
                <Input
                  id="publishedAt"
                  type="datetime-local"
                  value={publishedAt ? new Date(new Date(publishedAt).getTime() - new Date().getTimezoneOffset() * 60000).toISOString().slice(0, 16) : ""}
